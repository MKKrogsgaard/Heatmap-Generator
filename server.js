...
/*--OLD VERSION WITH RENAMING--*/
// async function handleFileUpload(request, response) {
//     if (!request.files || request.files.length === 0) { // Throw error if something went wrong with the file upload
//         return response.status(400).json({ error: 'No files uploaded' });
//     }
    
//     const renamePromises = request.files.map(file => {
//         const savedPath = file.path; // The temporary path generated by Multer, random filename, no extension
//         const originalName = file.originalname;
//         const finalPath = path.join(uploadDir, originalName); // Desired final path
        
//         return fs.promises.rename(savedPath, finalPath)
//             .then(() => ({file: originalName, savedAs: finalPath}))
//             .catch(err => ({file: originalName, error: err.message}));
//     });
